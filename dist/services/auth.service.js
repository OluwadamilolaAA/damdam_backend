...
const register = async (input) => {
    const existingUser = await user_model_1.UserModel.findOne({ email: input.email }).exec();
    if (existingUser) {
...
const login = async (input) => {
    const user = await user_model_1.UserModel.findOne({ email: input.email }).exec();
    if (!user) {
...
const logout = async (userId) => {
    await user_model_1.UserModel.findByIdAndUpdate(userId, { $set: { refreshTokenHashes: [] } }).exec();
};
...
const forgotPassword = async (email) => {
    const user = await user_model_1.UserModel.findOne({ email }).exec();
